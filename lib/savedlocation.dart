...
class _SavedAddressesPageState extends State<SavedAddressesPage> {
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Center(child: Text("Error loading addresses", style: GoogleFonts.inter(color: Colors.red)));
          }
        ...
        child: Row(
          ...
          children: [
            ...
            Container(
              ...
              child: Icon(Icons.location_on_rounded, color: _accentPink, size: 24),
            ),
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      InkWell(
                        onTap: () async {
                          await FirebaseFirestore.instance.collection('addresses').doc(docId).delete();
                        },
                  ...
                  Text(
                    data['address'] ?? "",
                    style: GoogleFonts.inter(color: Colors.grey[600], height: 1.4, fontSize: 13),
                  ),
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        data['phone'] ?? "",
                        style: GoogleFonts.inter(color: _premiumBlack, fontWeight: FontWeight.bold, fontSize: 13),
                      ),
                    ],
                  ),
                ],
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(Icons.add_location_alt_outlined, size: 60, color: Colors.grey[400]),
          ),
          const SizedBox(height: 20),
          Text(
            "NO ADDRESSES YET",
            style: GoogleFonts.fahkwang(fontWeight: FontWeight.bold, color: Colors.black26, fontSize: 18),
          ),
          const SizedBox(height: 10),
          const Text("Add a location for faster checkout!", style: TextStyle(color: Colors.black26)),
        ],
      ),
    );
  }

  // 🟢 BOTTOM SHEET TO ADD ADDRESS
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            Text("Add New Address", style: GoogleFonts.fahkwang(fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 25),
            
            _buildTextField(_nameController, "Label (e.g. Home, Office)", Icons.bookmark_border),
            const SizedBox(height: 15),
            _buildTextField(_addressController, "Full Address (House No, Street)", Icons.home_outlined),
            const SizedBox(height: 15),
            Row(
              children: [
                Expanded(child: _buildTextField(_pincodeController, "Pincode", Icons.pin_drop_outlined, isNumber: true)),
                const SizedBox(width: 15),
                Expanded(child: _buildTextField(_phoneController, "Phone Number", Icons.phone_outlined, isNumber: true)),
              ],
            ),
                ...
                onPressed: () async {
                  if (_addressController.text.isNotEmpty && user != null) {
                    ...
                    await FirebaseFirestore.instance.collection('addresses').add({
                      'userId': user!.uid,
                      'tag': _nameController.text.isEmpty ? "Home" : _nameController.text,
  ...
  Widget _buildTextField(TextEditingController controller, String hint, IconData icon, {bool isNumber = false}) {
    return TextField(
      ...
      style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: _premiumBlack),
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: _accentPink, width: 1.5)),
