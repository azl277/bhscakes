...
class _LocationPickerState extends State<LocationPage> with TickerProviderStateMixin {
  ...
  // --- SETTINGS ---
  LatLng _shopLocation = const LatLng(9.9312, 76.2673); // Default Kochi
  double _deliveryRadius = 15000; // 15km default
  ...
  // Controls the UI state: Pinning vs Entering Details
  bool _isPinningMode = true; 
  String _selectedLabel = "Home"; // Stores the selected address label
  ...
  Future<void> _initializeData() async {
    try {
      final doc = await FirebaseFirestore.instance.collection('settings').doc('delivery_zone').get().timeout(const Duration(seconds: 4));
      if (doc.exists) {
  ...
  Future<void> _decodeAddress(LatLng pos) async {
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        String address = [title, sub, city].where((e) => e.isNotEmpty).toSet().join(", ");
        
        setState(() {
          _detectedAddress = address;
          _areaController.text = [sub, city].where((e) => e.isNotEmpty).join(", ");
        });
      }
    } catch (e) {
      // Ignored
  ...
  // 🟢 SHOW SAVED ADDRESSES BOTTOM SHEET WITH DELETE OPTION
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            // 🟢 CLICKABLE AREA TO SELECT ADDRESS
                            Expanded(
                              child: InkWell(
                                onTap: () {
                ...
                child: AnimatedContainer(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Icon(Icons.location_on, size: 50, color: _isServiceable ? _accentPink : Colors.grey),
                      AnimatedContainer(
                        duration: const Duration(milliseconds: 200),
                        height: 6,
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.location_on_rounded, color: _isServiceable ? _accentPink : Colors.grey),
              ),
              const SizedBox(width: 15),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(_detectedAddress, style: GoogleFonts.inter(fontSize: 14, color: Colors.black87, fontWeight: FontWeight.w600), maxLines: 2, overflow: TextOverflow.ellipsis),
                  ],
                ),
              ),
            ],
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                flex: 5,
                child: _buildInput(_phoneController, "Receiver's Number", Icons.phone_rounded, keyboardType: TextInputType.phone),
              ),
              const SizedBox(width: 10),
              ...
              if (_phoneController.text.isEmpty)
                    ...
                    onPressed: () {
                      ...
                      if (user != null && user.phoneNumber != null && user.phoneNumber!.isNotEmpty) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text("No phone number linked to this login account."))
                        );
                      }
                    },
                ...
                Expanded(
                  flex: 5,
                  child: _buildInput(_nameController, "Receiver's Name", Icons.person_outline_rounded),
                ),
            ],
          ),
  ...
  Widget _buildGlassBtn(IconData icon, VoidCallback onTap, {Color color = Colors.black87, String? label}) {
    return InkWell(
      ...
      child: Container(
        height: 48,
        padding: label != null ? const EdgeInsets.symmetric(horizontal: 16) : null,
  ...
  Future<void> _confirmAddress() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please Login to Save Address")));
      return;
    }
    ...
    if (house.isEmpty) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please enter House/Flat Number")));
       return;
    }
    if (phone.isEmpty) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please enter Receiver's Phone Number")));
       return;
    }
    if (name.isEmpty) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please enter Receiver's Name")));
       return;
    }
    ...
    showDialog(
      ...
      builder: (c) => const Center(child: CircularProgressIndicator(color: Color(0xFFFF2E74)))
    );
